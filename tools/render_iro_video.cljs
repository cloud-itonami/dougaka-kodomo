;; いろのうた 映像レンダラ(ADR-2607164500 Phase A #2)。
;;   nbb tools/render_iro_video.cljs <build-dir>
;; compose-scene 設計: cutout layer(メロ/ポポ/ミミ) + BPM 同期 bounce +
;; 歌詞と同期した色スウォッチ出現。timeline.json の :sections/:colors を消費。
(ns render-iro-video
  (:require ["fs" :as fs]
            ["path" :as path]
            ["child_process" :as cp]
...
(def W 1280) (def H 720) (def beat (/ 60.0 96))
...
(defn swatch-svg [nm]
  (let [[fill txt] (palette nm)]
    (str "<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 300 300'>"
         "<rect x='14' y='14' width='272' height='272' rx='44' fill='" fill "'"
         (when (= nm "しろ") " stroke='#E3D9CF' stroke-width='4'") "/>"
         "<text x='150' y='185' text-anchor='middle' font-family='Hiragino Sans, sans-serif'"
         " font-weight='800' font-size='64' fill='" txt "'>" nm "</text></svg>")))

;; --- backgrounds -------------------------------------------------------------
(defn bg-base [& body]
  (str "<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 1280 720'>"
       "<defs><linearGradient id='sky' x1='0' y1='0' x2='0' y2='1'>"
       "<stop offset='0' stop-color='#DDEBFF'/><stop offset='1' stop-color='#FFF6E8'/></linearGradient></defs>"
...
(def bg-title
  (bg-base
   "<g>" (apply str (for [[i c] (map-indexed vector ["#E64B3C" "#F5943B" "#FFD23F" "#5DBB63" "#3B82C4" "#FF8FB6"])]
                      (str "<circle cx='" (+ 250 (* i 150)) "' cy='210' r='30' fill='" c "'/>"))) "</g>"
   "<text x='640' y='340' text-anchor='middle' font-family='Hiragino Sans, sans-serif'"
   " font-weight='800' font-size='112' fill='#FF7FA8' stroke='#fff' stroke-width='14' paint-order='stroke'>いろのうた</text>"
   "<text x='640' y='420' text-anchor='middle' font-family='Hiragino Sans, sans-serif'"
   " font-weight='600' font-size='44' fill='#4A6FA5' stroke='#fff' stroke-width='8' paint-order='stroke'>あか・あお・きいろ…なにいろ?</text>"))

(def bg-meadow (bg-base))
(def bg-chorus
  (str "<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 1280 720'>"
       "<defs><linearGradient id='pk' x1='0' y1='0' x2='0' y2='1'>"
       "<stop offset='0' stop-color='#FFF0E9'/><stop offset='1' stop-color='#E9F5FF'/></linearGradient></defs>"
       "<rect width='1280' height='720' fill='url(#pk)'/>"
       "<path d='M240,720 A400,400 0 0 1 1040,720' fill='none' stroke='#E64B3C' stroke-width='28'/>"
       "<path d='M276,720 A364,364 0 0 1 1004,720' fill='none' stroke='#F5943B' stroke-width='28'/>"
       "<path d='M312,720 A328,328 0 0 1 968,720' fill='none' stroke='#FFD23F' stroke-width='28'/>"
       "<path d='M348,720 A292,292 0 0 1 932,720' fill='none' stroke='#5DBB63' stroke-width='28'/>"
       "<path d='M384,720 A256,256 0 0 1 896,720' fill='none' stroke='#3B82C4' stroke-width='28'/>"
       "<text x='640' y='150' text-anchor='middle' font-family='Hiragino Sans, sans-serif'"
       " font-weight='800' font-size='80' fill='#FF7FA8' stroke='#fff' stroke-width='12' paint-order='stroke'>いろいろ たのしいな!</text></svg>"))
...
(defn gen-assets! []
  ...
  (doseq [[svg out w] [["melo.svg" "melo-m.png" 260] ["melo.svg" "melo-l.png" 330]
    ...
    (png svg out w))
  (doseq [nm (keys palette)]
    (let [f (str "sw-" (hash nm))]
      (write-svg (str f ".svg") (swatch-svg nm))
      (png (str f ".svg") (str f "-m.png") 180)
      (png (str f ".svg") (str f "-big.png") 320)))
  (println "assets rendered"))

(defn sw-png [nm sz] (str "sw-" (hash nm) "-" sz ".png"))
...
(defn -main []
  ...
  (let [tl (js->clj (js/JSON.parse (fs/readFileSync (path/join build-dir "timeline.json") "utf8")) :keywordize-keys true)
        secs (:sections tl)
        sec-at (fn [i] (:start-sec (nth secs i)))
        v1 (nth secs 1) v2 (nth secs 3)
        x-of (fn [i n] (+ 130 (* (/ (- W 400) (max 1 (dec n))) i)))
        verse-chars [{:img "melo-m.png" :x 40 :y-expr (bounce 330 22)}
                     {:img "mimi-m.png" :x 1000 :y-expr (bounce 340 26)}]
        ...
        v1-colors ["あか" "あお" "きいろ" "みどり"]
        v2-colors ["しろ" "くろ" "ぴんく" "おれんじ"]]
    ;; intro
    (render-section! "sec0.mp4" "bg-title.png" (:dur-sec (nth secs 0))
                     [{:img "melo-m.png" :x 300 :y-expr (bounce 430 22)}
                      {:img "popo-m.png" :x 590 :y-expr (bounce 470 18)}
                      {:img "mimi-m.png" :x 830 :y-expr (bounce 440 26)}])
    ;; verse1: 4 swatches pop at local 0,4,8,12 beats (=color intervals)
    (render-section! "sec1.mp4" "bg-meadow.png" (:dur-sec v1)
                     (concat verse-chars
                             (map-indexed (fn [i nm]
                                            {:img (sw-png nm "m") :x (x-of i 4) :y-expr (bob 150 8 i)
                                             :enable (str "gte(t," (* i 4 beat) ")")}) v1-colors)))
    ;; chorus
    (render-section! "sec2.mp4" "bg-chorus.png" (:dur-sec (nth secs 2)) chorus-chars)
    ;; verse2
    (render-section! "sec3.mp4" "bg-meadow.png" (:dur-sec v2)
                     (concat verse-chars
                             (map-indexed (fn [i nm]
                                            {:img (sw-png nm "m") :x (x-of i 4) :y-expr (bob 150 8 i)
                                             :enable (str "gte(t," (* i 4 beat) ")")}) v2-colors)))
    ;; chorus2
    (render-section! "sec4.mp4" "bg-chorus.png" (:dur-sec (nth secs 4)) chorus-chars)
    ;; outro: all 8 swatches in a row + big current
    (render-section! "sec5.mp4" "bg-meadow.png" (:dur-sec (nth secs 5))
                     (concat [{:img "melo-m.png" :x 160 :y-expr (bounce 350 22)}
                             ...
                             (map-indexed (fn [i nm]
                                            {:img (sw-png nm "m") :x (+ 40 (* 150 i)) :y-expr (bob 90 6 i)})
                                          (concat v1-colors v2-colors))))
    ;; concat + mux
    ...
    (sh "ffmpeg" ["-i" (path/join build-dir "video-noaudio.mp4")
                  ...
                  (path/join build-dir "iro-no-uta.mp4")])
    (println "iro-no-uta.mp4 done")))
