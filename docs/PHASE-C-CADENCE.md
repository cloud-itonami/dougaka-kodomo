...
- **headless SSH セッションには `launchctl bootstrap`(gui/user domain)が使えない**
  ...
  `tools/cadence.cljs` は起動前に `ensure-voicevox!` で自己ヘルスチェック
...
- **同じ理由で macOS Keychain も headless SSH からは書き込めない**
  ...
  `tools/publish_aozora.cljs` の actor seed 解決は
  ...
  新規生成させない。
- `tools/produce.cljs` の kotobase-client パスは
...
```
ssh asher@100.96.122.69 '(crontab -l 2>/dev/null; echo "0 10 * * * cd \$HOME/orgs/gftdcojp/ai-gftd-dougaka-kodomo && PATH=/opt/homebrew/bin:/usr/bin:/bin:/usr/local/bin /opt/homebrew/bin/nbb --classpath src:resources tools/cadence.cljs --publish >> /tmp/dougaka-kodomo-cadence.log 2>&1") | crontab -'
